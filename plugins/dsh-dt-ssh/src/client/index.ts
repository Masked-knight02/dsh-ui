/**
 * dsh-dt-ssh browser half: a sidebar entry row injected next to the shell's
 * New Session button (DOM-level extension, self-healing via a
 * MutationObserver) and an SSH operations panel (host CRUD, connect test,
 * command exec) over the host /api/dsh-dt-ssh routes.
 *
 * @module @masked-knight02/dsh-client-ui-dt-ssh/client
 */
...
export const ENTRY_SELECTOR = '[data-dsh-dt-ssh-entry]'

/** Inline icon matching the shell's 16px nav-icon look. */
const ICON = `<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2.5 10.5c0-2 1.2-4 2.5-4s2 2.6 2 4c0 1.1-.7 1.5-1.2 1.5S5.6 10.9 5 10.5s-.4-.2-.8-.4C3.7 9.9 3.5 10 3.5 9.5z"/><path d="M8 4.2c-.4-1-1.3-1.7-2.4-1.7"/></svg>`
...
/** The entry row toggles the SSH panel. */
function createEntry(onToggle: () => void): HTMLButtonElement {
  const entry = document.createElement('button')
  entry.type = 'button'
  entry.dataset.dshDtSshEntry = ''
  ...
  entry.innerHTML = `<span style="display:inline-flex">${ICON}</span><span>SSH 运维</span>`
...
/**
 * Mount the sidebar entry and the SSH panel.
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => {
    ...
    const openPanel = (): void => {
      ...
      panelHost.id = 'dsh-dt-ssh-panel-host'
      document.body.appendChild(panelHost)
      root = createRoot(panelHost)
      root.render(createElement(SshPanel, { onClose: closePanel }))
    }
